...
class UnmixGAN():
    ...
    def __init__(self, args, dataloaders):
        ...
        self._exclusion_loss = loss.ExclusionLoss()  # 排除损失
        self._kurtosis_loss = loss.KurtosisLoss()  # 峰值损失
        # enable some losses?                       #启用损失?
    ...
    def _collect_epoch_states(self):
        ...
        print('Is_training: %s. Epoch %d / %d, epoch_acc= %.8f (%s),' %
              (self.is_training, self.epoch_id, self.max_num_epochs - 1, self.epoch_acc, self.metric))  # 再加一个指标
        print()
    ...
    def _update_checkpoints(self):
        ...
        print('Lastest model updated. Epoch_acc=%.4f, Historical_best_acc=%.4f (at epoch %d)'
              % (self.epoch_acc, self.best_val_acc, self.best_epoch_id))  # 再加一个指标
        print()
